...
namespace PBL3.GUI
{
    public partial class noti_CQTQ : Form
    {
        public delegate void Mydel();
        public Mydel dlg { get; set; }
        private CQTQ current;
        public noti_CQTQ(LDVN l)
        {
            InitializeComponent();
        ...
        private void cbbTP_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            cbbreceiver.Items.Clear();
            cbbreceiver.Items.Add("All");
            cbbreceiver.Items.AddRange(BLL_QLCD.Instance.GetListIDCDVN(cbbTP.Text, cbbQH.Text, cbbPX.Text).ToArray());
        }
        ...
        private void bsent_Click(object sender, EventArgs e)
        {
            ...
            if(cbbreceiver.SelectedItem != null && cbbtitle.SelectedItem != null)
            {
                if (cbbreceiver.SelectedItem.ToString() == "All")
                {
                    foreach(string s in cbbreceiver.Items)
                    {
                        if (s != "All")
                        {
                ...
                else
                {
                    ...
                    {
                        Title = cbbtitle.SelectedItem.ToString(),
                        ID_CD = cbbreceiver.SelectedItem.ToString(),
                        Date_sent = DateTime.Now,
                        ND = txtND.Text,
                        ID_CQTQ = current.ID_CQTQ
                    };
